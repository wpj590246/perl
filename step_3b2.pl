...
sub fill_csv{	
	...
	my $sql = "delete from spider_weng_b1 where uid =''" ;
	$dbh->do($sql);
while (1) {
    
	$sql = "select * from spider_weng_b1 where flag = 0 order by id limit 10000";
    ...
    $dbh->do("update spider_weng_b1 set flag = 1 where flag = 0 order by id limit 10000");
